...
public class TaskDecompositionAgent extends BaseModelAgent<TaskPlan> {
    ...
    protected TaskPlan map(AgentTask task, ExecutionContext context, Map<String, String> fields) {
        List<TaskPlan.PlannedTask> tasks = List.of(
            ...
            new TaskPlan.PlannedTask("synchronize-patch", AgentRole.PLANNER, "Synchronize implementation and test proposals before policy evaluation", List.of("implement-change", "generate-tests")),
            ...
            new TaskPlan.PlannedTask("release-readiness", AgentRole.RELEASE_REVIEWER, "Review validation, documentation, and risk evidence", List.of("synchronize-patch", "security-risk-review"))
        );
